...
ROperation operator%(const ROperation& op1,const ROperation& op2)
{
    if(op1.op==Num&&op2.op==Num)return (int)op1.ValC%(int)op2.ValC;
    if(op1==0.)return op2;if(op2==0.)return op1;
    if(op1.op==Opp)return op2-*(op1.mmb2);if(op2.op==Opp)return op1-*(op2.mmb2);
    ROperation resultat;
    resultat.op=Add;resultat.mmb1=new ROperation(op1);
...
int SearchOperator(char*s,ROperator op)
{
  ...
  for(i=(int)strlen(s)-1;i>=0;i--){
    ...
    if(s[i]==')'){i=SearchCorClosebracket(s,i);
        if(i==-1)
        {
            return -1;
        }
...
ROperation::ROperation(char*sp,int nvar,PRVar*ppvarp,int nfuncp,PRFunction*ppfuncp)
{
  ...
  mmb2=new ROperation(s2,nvar,ppvarp,nfuncp,ppfuncp);goto fin;
    ...
    i=SearchOperator(s,Mod);
    if(i!=-1){s1=MidStr(s,0,i-1);s2=MidStr(s,i+1,strlen(s)-1);
        op=Mod;mmb1=new ROperation(s1,nvar,ppvarp,nfuncp,ppfuncp);
        mmb2=new ROperation(s2,nvar,ppvarp,nfuncp,ppfuncp);goto fin;
    };
...
void ROperation::BuildCode()
{
  ...
  break;case Mod:BCDouble(pinstr,mmb1->pinstr,mmb2->pinstr,
              pvals,mmb1->pvals,mmb2->pvals,ppile,mmb1->ppile,mmb2->ppile,pfuncpile,mmb1->pfuncpile,mmb2->pfuncpile,&ModFunc);
